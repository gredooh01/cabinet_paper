        ...
        <div class="col-md-12">
            @can('cabinet_paper_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.cabinet-papers.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $cabinetPaper->id }}">
                                        ...
                                        <td>
                                            @can('cabinet_paper_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.cabinet-papers.show', $cabinetPaper->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('cabinet_paper_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.cabinet-papers.edit', $cabinetPaper->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('cabinet_paper_delete')
                                                <form action="{{ route('frontend.cabinet-papers.destroy', $cabinetPaper->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cabinet_paper_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.cabinet-papers.massDestroy') }}",
